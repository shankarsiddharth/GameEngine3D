...
GE3D::cResult GE3D::Assets::cMeshBuilder::Build(const std::vector<std::string>& i_arguments)
{
	auto result = Results::Success;

	// Read the json file
	GE3D::Platform::sDataFromFile dataFromFile;
	GE3D::Platform::LoadBinaryFile(m_path_source, dataFromFile);
	...
	auto* vertexArray = new GE3D::Graphics::VertexFormats::sVertex_mesh[maxVertexCount];
		...
		//Parse Vertex Data
		{
			...
			if (vertexDataArray.is_array())
			{
				for (const auto& vertexData : vertexDataArray)
				{
					...
					{
						GE3D::Graphics::VertexFormats::sVertex_mesh vertex{};
	...
	else
	{
		...
		const size_t vertexArraySize = sizeof(GE3D::Graphics::VertexFormats::sVertex_mesh) * (static_cast<size_t>(vertexArrayIndex));
