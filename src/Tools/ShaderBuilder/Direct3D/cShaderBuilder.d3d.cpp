...
GE3D::cResult GE3D::Assets::cShaderBuilder::Build( const Graphics::eShaderType i_shaderType, const std::vector<std::string>& i_arguments )
{
	...
	// Compile it
	{
		...
		const auto* const targetProfile = [this, i_shaderType]
		{
			...
			case Graphics::eShaderType::Fragment:
				...
				break;
			default:
				GE3D_ASSERTF( false, "Invalid shader type" );
				GE3D::Assets::OutputErrorMessageWithFileInfo( m_path_source,
	...
	{
		std::string errorMessage;
		if ( !( result = GE3D::Platform::WriteBinaryFile( m_path_target, compiledCode->GetBufferPointer(), compiledCode->GetBufferSize(), &errorMessage ) ) )
		{
			GE3D::Assets::OutputErrorMessageWithFileInfo( m_path_source, errorMessage.c_str() );
		}
	}
...
#define GE3D_GRAPHICS_SHOULDDISASSEMBLYBECREATED
#ifdef GE3D_GRAPHICS_SHOULDDISASSEMBLYBECREATED
		const auto path_disassembly = [i_shaderType]( const std::string& i_path_compiledShader )
		{
			const auto directory_disassembly = [i_shaderType, &i_path_compiledShader]()
			{
				...
				if ( !GE3D::Platform::GetEnvironmentVariable( "IntermediateDir", directory_disassembly ) )
				{
		...
		{
			std::string errorMessage;
			if ( !GE3D::Platform::CreateDirectoryIfItDoesntExist( path_disassembly, &errorMessage ) )
			{
				GE3D::Assets::OutputWarningMessageWithFileInfo( path_disassembly.c_str(), errorMessage.c_str() );
			}
			if ( !GE3D::Platform::WriteBinaryFile( path_disassembly.c_str(),
...
namespace
{
	...
	GE3D::cResult cIncludeHelper::Initialize()
	{
		...
		const void** o_dataFromFile, unsigned int* o_dataSize )
	{
		...
		GE3D_ASSERT( o_dataFromFile != nullptr );
		GE3D_ASSERT( o_dataSize != nullptr );

		GE3D::Platform::sDataFromFile dataFromFile;
		GE3D::cScopeGuard scopeGuard( [o_dataFromFile, o_dataSize, &result, &dataFromFile]
			{
				if ( SUCCEEDED( result ) )
				{
					GE3D_ASSERT( dataFromFile.data != nullptr );
					GE3D_ASSERT( *o_dataFromFile == nullptr );				
					*o_dataFromFile = dataFromFile.data;
					dataFromFile.data = nullptr;
					GE3D_ASSERT( dataFromFile.size < std::numeric_limits<std::remove_pointer<decltype( o_dataSize )>::type>::max() );
			...
			case D3D_INCLUDE_LOCAL:
				{
					...
					if ( !GE3D::Platform::DoesFileExist( pathToOpen.c_str() ) )
					{
						result = ERROR_PATH_NOT_FOUND;
						GE3D::Assets::OutputErrorMessageWithFileInfo( m_shaderSourcePath.c_str(),
		...
		{
			std::string errorMessage;
			const auto result_load = GE3D::Platform::LoadBinaryFile( pathToOpen.c_str(), dataFromFile, &errorMessage );
			if ( !result_load )
			{
				result = E_FAIL;
