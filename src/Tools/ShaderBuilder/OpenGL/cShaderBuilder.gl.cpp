...
namespace
{
	GE3D::cResult BuildAndVerifyGeneratedShaderSource( const char* const i_path_source, const char* const i_path_target,
		const GE3D::Graphics::eShaderType i_shaderType, const std::string& i_source );
	GE3D::cResult PreProcessShaderSource( const char* const i_path_source, std::string& o_shaderSource_preProcessed );
	GE3D::cResult SaveGeneratedShaderSource( const char* const i_path, const std::string& i_source );
}
...
GE3D::cResult GE3D::Assets::cShaderBuilder::Build( const Graphics::eShaderType i_shaderType, const std::vector<std::string>& i_arguments )
{
...
namespace
{
		...
		const GE3D::Graphics::eShaderType i_shaderType, const std::string& i_source )
	{
		auto result = GE3D::Results::Success;
		{
			...
			GE3D::Windows::OpenGl::sHiddenWindowInfo hiddenWindowInfo;
			GE3D::cScopeGuard scopGuard_hiddenWindowInfo( [i_path_source, &result, &applicationInstance, &hiddenWindowInfo]
				{
					std::string errorMessage;
					const auto result_hiddenWindowInfo = GE3D::Windows::OpenGl::FreeHiddenContextWindow( applicationInstance, hiddenWindowInfo, &errorMessage );
					if ( !result_hiddenWindowInfo )
					{
						...
						GE3D::Assets::OutputErrorMessageWithFileInfo( i_path_source, errorMessage.c_str() );
					}
				} );
			...
			{
				std::string errorMessage;
				if ( !( result = GE3D::OpenGlExtensions::Load( &errorMessage ) ) )
				{
					GE3D::Assets::OutputErrorMessageWithFileInfo( i_path_source, errorMessage.c_str() );
					return result;
				}
			...
			{
				std::string errorMessage;
				if ( !( result = GE3D::Windows::OpenGl::CreateHiddenContextWindow( applicationInstance, hiddenWindowInfo, &errorMessage ) ) )
				{
					GE3D::Assets::OutputErrorMessageWithFileInfo( i_path_source, errorMessage.c_str() );
					return result;
				}
			...
			// Determine which vendor makes the GPU
			{
				...
				else
				{
					result = GE3D::Results::Failure;
					{
						...
						GE3D::Assets::OutputErrorMessageWithFileInfo( i_path_source, errorMessage.str().c_str() );
					}
					return result;
				}
			}
			// Verify that compiling shaders at run-time is supported
			{
				...
				{
					result = GE3D::Results::Failure;
					GE3D::Assets::OutputErrorMessageWithFileInfo( i_path_source,
			...
			GLuint shaderId = 0;
			GE3D::cScopeGuard scopeGuard_shaderId( [i_path_source, &result, &shaderId]
				{
					if ( shaderId != 0 )
					{
				...
				} );
			{
				...
				switch ( i_shaderType )
				{
				case GE3D::Graphics::eShaderType::Vertex:
					shaderType = GL_VERTEX_SHADER;
					break;
				case GE3D::Graphics::eShaderType::Fragment:
				...
				shaderId = glCreateShader( shaderType );
				{
					...
					{
						result = GE3D::Results::Failure;
						GE3D::Assets::OutputErrorMessageWithFileInfo( i_path_source, "OpenGL failed to get an unused shader ID" );
						return result;
					}
				}
	...
	GE3D::cResult PreProcessShaderSource( const char* const i_path_source, std::string& o_shaderSource_preProcessed )
	{
		...
		std::string includeSearchPathArgument_engineSourceContentDir, includeSearchPathArgument_gameSourceContentDir;
		{
			...
			{
				if ( result = GE3D::Platform::GetEnvironmentVariable(
				...
				{
					GE3D::Assets::OutputErrorMessage( "Failed to get the engine's source content directory: %s", errorMessage.c_str() );
					return result;
				}
			}
			// GameSourceContentDir
			{
				...
				{
					GE3D::Assets::OutputErrorMessage( "Failed to get the game's source content directory: %s", errorMessage.c_str() );
					return result;
				}
			}
		}
...
#ifdef GE3D_GRAPHICS_AREDEBUGSHADERSENABLED
			// Keep comments
			"-C",
#endif
		...
		GE3D::cScopeGuard scopeGuard_arguments( [&arguments, &allocatedArgumentCount]
			{
		...
		for ( size_t i = 0; i < argumentCount; ++i )
		{
			...
			{
				result = GE3D::Results::OutOfMemory;
				GE3D::Assets::OutputErrorMessage( "Failed to allocate &u bytes of temporary storage for argument #&u (\"%s\")",
		...
		GE3D::cScopeGuard scopeGuard_mcppBuffers( []
			{
		...
		// Preprocess the file
		{
			...
			{
				const auto* const shaderSource_preProcessed = mcpp_get_mem_buffer( static_cast<OUTDEST>( GE3D::mcpp::OUTDEST::Out ) );
				o_shaderSource_preProcessed = shaderSource_preProcessed ? shaderSource_preProcessed : "";
			}
			else
			{
				result = GE3D::Results::Failure;
				std::cerr << mcpp_get_mem_buffer( static_cast<OUTDEST>( GE3D::mcpp::OUTDEST::Err ) );
				return result;
			}
		}

#ifndef GE3D_GRAPHICS_AREDEBUGSHADERSENABLED
	...
	GE3D::cResult SaveGeneratedShaderSource( const char* const i_path, const std::string& i_shader )
	{
		GE3D::cResult result;
		std::string errorMessage;
		if ( !( result = GE3D::Platform::WriteBinaryFile( i_path, i_shader.c_str(), i_shader.length(), &errorMessage ) ) )
		{
			GE3D::Assets::OutputErrorMessageWithFileInfo( i_path, errorMessage.c_str() );
		}
		return result;
	}
}
