...
namespace
{
	...
	private:
		...
		// Initialize / Clean Up
		GE3D::cResult Initialize();
		GE3D::cResult CleanUp();

		GE3D::cResult SaveReferenceToGlobalFunctionInRegistry( const char* const i_functionName, int &o_functionReference );
	};
}
...
namespace
{
	...
	GE3D::cResult FormatErrorOrWarningMessage( const char* const i_message, std::string &o_message, va_list io_insertions );
...
GE3D::cResult GE3D::Assets::BuildAssets( const char* const i_path_assetsToBuild )
{
	return s_luaState.BuildAssets( i_path_assetsToBuild );
}

GE3D::cResult GE3D::Assets::ConvertSourceRelativePathToBuiltRelativePath( const char* const i_sourceRelativePath, const char* const i_assetType,
	std::string& o_builtRelativePath, std::string* o_errorMessage )
{
...
void GE3D::Assets::OutputErrorMessage( const char* const i_errorMessage, ... )
{
...
void GE3D::Assets::OutputErrorMessageWithFileInfo( const char* const i_filePath,
...
void GE3D::Assets::OutputWarningMessage( const char* const i_warningMessage, ... )
{
	std::string formattedMessage;
	GE3D::cResult result;
	{
...
void GE3D::Assets::OutputWarningMessageWithFileInfo( const char* const i_filePath,
...
namespace
{
	...
	GE3D::cResult cLuaState::BuildAssets( const char* const i_path_assetsToBuild )
	{
		auto result = GE3D::Results::Success;

		GE3D::cScopeGuard scopeGuard( [this]
			{
				GE3D_ASSERT( !luaState || ( lua_gettop( luaState ) == 0 ) );
			} );

		// Initialize the Lua environment if necessary
		if ( luaState || ( result = Initialize() ) )
		{
			GE3D_ASSERT( lua_gettop( luaState ) == 0 );

			// Push the Lua function on the stack
			GE3D_ASSERT( ( m_fBuildAssets != LUA_NOREF ) && ( m_fBuildAssets != LUA_REFNIL ) );
			...
			if ( type == LUA_TFUNCTION )
			{
				...
				// Check the return values
				{
					...
					{
						result = lua_toboolean( luaState, -1 ) ? GE3D::Results::Success : GE3D::Results::Failure;
					}
					else
					{
						result = GE3D::Results::Failure;
						{
							GE3D::Assets::OutputErrorMessage( "BuildAssets() should return a boolean as return value #1, not a %s",
			...
			{
				result = GE3D::Results::Failure;
				GE3D::Assets::OutputErrorMessageWithFileInfo( __FILE__, __LINE__,
	...
	GE3D::cResult cLuaState::ConvertSourceRelativePathToBuiltRelativePath( const char* const i_sourceRelativePath, const char* const i_assetType,
		std::string& o_builtRelativePath, std::string* o_errorMessage )
	{
		auto result = GE3D::Results::Success;
		...
		if ( luaState || ( result = Initialize() ) )
		{
			...
			// Push the Lua function on the stack
			GE3D_ASSERT( ( m_fConvertSourceRelativePathToBuiltRelativePath != LUA_NOREF ) && ( m_fConvertSourceRelativePathToBuiltRelativePath != LUA_REFNIL ) );
			...
			if ( type == LUA_TFUNCTION )
			{
				...
				// Check the return values
				{
					if ( lua_isboolean( luaState, -2 ) )
					{
						...
						if ( !wereThereErrors )
						{
							...
							else
							{
								...
								{
									GE3D::Assets::OutputErrorMessage(
						...
						else
						{
							...
							if ( lua_isstring( luaState, -1 ) )
							{
								...
								{
									GE3D::Assets::OutputErrorMessage( lua_tostring( luaState, -1 ) );
								}
							}
							else
							{
					...
					else
					{
						...
						{
							GE3D::Assets::OutputErrorMessage( "ConvertSourceRelativePathToBuiltRelativePath() should return a boolean as return value #1, not a %s",
								luaL_typename( luaState, -2 ) );
						}
					}
					// Pop the returned values
	...
	GE3D::cResult cLuaState::Initialize()
	{
		auto result = GE3D::Results::Success;

		GE3D::cScopeGuard scopeGuard( [this, &result]
			{
				if ( luaState )
				{
					GE3D_ASSERT( lua_gettop( luaState ) == 0 );
		...
		// Set the platform #defines
		{
			...
			lua_setglobal( luaState, "GE3D_PLATFORM_WINDOWS" );

			#if defined( GE3D_PLATFORM_D3D )
				lua_pushboolean( luaState, true );
				lua_setglobal( luaState, "GE3D_PLATFORM_D3D" );
			#elif defined( GE3D_PLATFORM_GL )
				lua_pushboolean( luaState, true );
				lua_setglobal( luaState, "GE3D_PLATFORM_GL" );
			#endif
		#endif
		}
		// Load the Lua asset build functions
		{
			...
			std::string path;
			{
				// Get the output directory
				{
					...
					if ( !( result = GE3D::Platform::GetEnvironmentVariable( key, path, &errorMessage ) ) )
					{
						GE3D::Assets::OutputErrorMessageWithFileInfo( __FILE__, __LINE__, errorMessage.c_str() );
						return result;
					}
				}
			...
			{
				std::string errorMessage;
				if ( GE3D::Platform::DoesFileExist( path.c_str(), &errorMessage ) )
				{
					...
					if ( luaResult == LUA_OK )
					{
						...
						if ( luaResult != LUA_OK )
						{
							...
							default:
								{
									result = GE3D::Results::InvalidFile;
								}
							}
							return result;
						}
				...
				{
					result = GE3D::Results::FileDoesntExist;
					GE3D::Assets::OutputErrorMessageWithFileInfo( __FILE__, __LINE__, errorMessage.c_str() );
					return result;
				}
			}
	...
	GE3D::cResult cLuaState::CleanUp()
	{
		auto result = GE3D::Results::Success;
	...
	GE3D::cResult cLuaState::SaveReferenceToGlobalFunctionInRegistry( const char* const i_functionName, int &o_functionReference )
	{
		auto result = GE3D::Results::Success;
		...
		if ( type == LUA_TFUNCTION )
		{
			...
			GE3D_ASSERT( o_functionReference != LUA_REFNIL );
		}
		else
		{
			result = GE3D::Results::Failure;
			{
				...
				GE3D::Assets::OutputErrorMessageWithFileInfo( __FILE__, errorMessage.str().c_str() );
			}
			lua_pop( luaState, 1 );
		}
...
namespace
{
	...
	GE3D::cResult FormatErrorOrWarningMessage( const char* const i_message, std::string &o_message, va_list io_insertions )
	{
		...
		if ( formattingResult >= 0 )
		{
			...
			{
				GE3D_ASSERTF( false, "The internal asset build buffer of size %u isn't big enough to hold the formatted message of length %i",
		...
		{
			GE3D_ASSERTF( false, "An encoding error occurred while formatting the asset build message \"%s\"", i_message );
			std::ostringstream errorMessage;
	...
	{
#if defined( GE3D_PLATFORM_WINDOWS )
		GE3D::Windows::OutputErrorMessageForVisualStudio( i_errorMessage, i_optionalFilePath, i_optionalLineNumber, i_optionalColumnNumber );
#else
	...
	{
#if defined( GE3D_PLATFORM_WINDOWS )
		GE3D::Windows::OutputWarningMessageForVisualStudio( i_warningMessage, i_optionalFilePath, i_optionalLineNumber, i_optionalColumnNumber );
#else
	...
	int luaCopyFile( lua_State* io_luaState )
	{
		...
		// Copy the file
		{
			...
			if ( GE3D::Platform::CopyFile( i_path_source, i_path_target, noErrorIfTargetAlreadyExists, updateTheTargetFileTime, &errorMessage ) )
			{
	...
	int luaCreateDirectoryIfItDoesntExist( lua_State* io_luaState )
	{
		...
		std::string errorMessage;
		if ( GE3D::Platform::CreateDirectoryIfItDoesntExist( i_path, &errorMessage ) )
		{
			constexpr int returnValueCount = 0;
			return returnValueCount;
		}
	...
	int luaDoesFileExist( lua_State* io_luaState )
	{
		...
		std::string errorMessage;
		if ( GE3D::Platform::DoesFileExist( i_path, &errorMessage ) )
		{
			lua_pushboolean( io_luaState, true );
			constexpr int returnValueCount = 1;
			return returnValueCount;
	...
	int luaExecuteCommand( lua_State* io_luaState )
	{
		...
		int exitCode;
		std::string errorMessage;
		if ( GE3D::Platform::ExecuteCommand( i_command, &exitCode, &errorMessage ) )
		{
			lua_pushboolean( io_luaState, true );
	...
	int luaGetEnvironmentVariable( lua_State* io_luaState )
	{
		...
		std::string value;
		std::string errorMessage;
		if ( GE3D::Platform::GetEnvironmentVariable( i_key, value, &errorMessage ) )
		{
	...
	int LuaGetFilesInDirectory( lua_State* io_luaState )
	{
		...
		std::string errorMessage;
		if ( GE3D::Platform::GetFilesInDirectory( i_path, paths, i_shouldSubdirectoriesBeSearchedRecursively, &errorMessage ) )
		{
			const auto arraySize = paths.size();
			GE3D_ASSERT( arraySize <= static_cast<decltype( arraySize )>( std::numeric_limits<int>::max() ) );
	...
	int luaGetLastWriteTime( lua_State* io_luaState )
	{
		...
		std::string errorMessage;
		if ( GE3D::Platform::GetLastWriteTime( i_path, lastWriteTime, &errorMessage ) )
		{
	...
	int luaInvalidateLastWriteTime( lua_State* io_luaState )
	{
		...
		std::string errorMessage;
		if ( GE3D::Platform::InvalidateLastWriteTime( i_path, &errorMessage ) )
		{
			constexpr int returnValueCount = 0;
			return returnValueCount;
		}
	...
	int luaOutputErrorMessage( lua_State* io_luaState )
	{
		...
		{
			GE3D::Assets::OutputErrorMessageWithFileInfo( i_optionalFileName, i_errorMessage );
		}
		else
		{
			GE3D::Assets::OutputErrorMessage( i_errorMessage );
		}
	...
	int luaOutputWarningMessage( lua_State* io_luaState )
	{
		...
		if ( i_optionalFileName )
		{
			GE3D::Assets::OutputWarningMessageWithFileInfo( i_optionalFileName, i_warningMessage );
		}
		else
		{
			GE3D::Assets::OutputWarningMessage( i_warningMessage );
		}
