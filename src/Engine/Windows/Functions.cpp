...
std::wstring GE3D::Windows::ConvertUtf8ToUtf16( const char* const i_string_utf8 )
{
	...
	constexpr int processEntireString = -1;
	{
		...
		if ( requiredCodeUnitCount_includingNullTerminator == 0 )
		{
			...
			GE3D_ASSERTF( false, "The number of code units required to convert \"%s\""
				" to UTF16 wasn't calculated: %s", i_string_utf8, GetFormattedSystemMessage( errorCode ).c_str() );
			GE3D::Logging::OutputError( "Windows couldn't calculate how many code units"
	...
	if ( requiredCodeUnitCount_includingNullTerminator > 1 )
	{
		...
		// Convert the string
		{
			...
			{
				const auto errorCode = GetLastError();
				GE3D_ASSERTF( false, "The UTF-8 string \"%s\""
					" wasn't converted to UTF-16: %s", i_string_utf8, GetFormattedSystemMessage( errorCode ).c_str() );
				GE3D::Logging::OutputError( "Windows couldn't convert a UTF-8 string to UTF-16: %s" "\n"
					"\t" "The UTF-8 string is:" "\n"
					"%s",
					GetFormattedSystemMessage( errorCode ).c_str(), i_string_utf8 );
				return L"INVALIDCONVERSION";
...
std::string GE3D::Windows::ConvertUtf16ToUtf8( const wchar_t* const i_string_utf16 )
{
	...
	constexpr BOOL* const wasDefaultCharacterUsed = nullptr;	// This must be NULL for UTF-8
	{
		...
		{
			const auto errorCode = GetLastError();
			GE3D_ASSERTF( false, "The number of code units required to convert a UTF-16 string"
	...
	if ( requiredCodeUnitCount_includingNullTerminator > 1 )
	{
		...
		// Convert the string
		{
			...
			{
				const auto errorCode = GetLastError();
				GE3D_ASSERTF( false, "A UTF-16 string"
					" wasn't converted to UTF-8: %s", GetFormattedSystemMessage( errorCode ).c_str() );
				GE3D::Logging::OutputError( "Windows couldn't convert a UTF-16 string to UTF-8: %s",
					GetFormattedSystemMessage( errorCode ).c_str() );
				return "INVALIDCONVERSION";
			}
		}
...
GE3D::cResult GE3D::Windows::CopyFile( const char* const i_path_source, const char* const i_path_target,
...
GE3D::cResult GE3D::Windows::CreateDirectoryIfItDoesntExist( const std::string& i_filePath, std::string* const o_errorMessage )
{
...
bool GE3D::Windows::DoesFileExist( const char* const i_path, std::string* const o_errorMessage )
{
	...
	if ( file != INVALID_HANDLE_VALUE )
	{
		...
		{
			const auto errorCode = GetLastError();
			GE3D_ASSERTF( false, "Windows failed to close the file handle to \"%s\" after finding it: %s",
	...
	{
		const auto errorCode = GetLastError();
		GE3D_ASSERTF( ( ( errorCode == ERROR_FILE_NOT_FOUND ) || ( errorCode == ERROR_PATH_NOT_FOUND ) ),
...
GE3D::cResult GE3D::Windows::ExecuteCommand( const char* const i_command, DWORD* const o_exitCode, std::string* const o_errorMessage )
{
	constexpr char* const noPath = nullptr;
	return ExecuteCommand( noPath, i_command, o_exitCode, o_errorMessage );
}

GE3D::cResult GE3D::Windows::ExecuteCommand( const char* const i_path, const char* const i_optionalArguments,
	DWORD* const o_exitCode, std::string* const o_errorMessage )
{
			...
			else
			{
				...
				if ( quoteCountAfterCommand < quoteCountBeginning )
				{
						...
						quoteCountToRemoveFromEnd > 0; --quoteCountToRemoveFromEnd )
					{
						...
						{
							GE3D_ASSERTF( false, "Expected a trailing quote but didn't find it" );
							break;
						}
					}
				}
		...
		&startupInfo, &processInformation ) != FALSE )
	{
		...
		if ( WaitForSingleObject( processInformation.hProcess, INFINITE ) != WAIT_FAILED )
		{
			...
			if ( o_exitCode )
			{
				if ( GetExitCodeProcess( processInformation.hProcess, o_exitCode ) == FALSE )
				{
					...
					GE3D_ASSERTF( false, "Couldn't get exit code of a process: %s", GetFormattedSystemMessage( errorCode ).c_str() );
					if ( o_errorMessage )
					{
						std::ostringstream errorMessage;
		...
		else
		{
			const auto errorCode = GetLastError();
			result = Results::Failure;
			GE3D_ASSERTF( false, "Didn't wait for a process to finish: %s", GetFormattedSystemMessage( errorCode ).c_str() );
			if ( o_errorMessage )
			{
				std::ostringstream errorMessage;
		...
		if ( CloseHandle( processInformation.hProcess ) == FALSE )
		{
			...
			GE3D_ASSERTF( false, "Windows failed to close the handle to the process \"%s\""
		...
		if ( CloseHandle( processInformation.hThread ) == FALSE )
		{
			...
			GE3D_ASSERTF( false, "Windows failed to close the handle to the process \"%s\" thread"
				" after executing a command: %s", path.c_str(), GetFormattedSystemMessage( errorCode ).c_str() );
			if ( o_errorMessage )
			{
				std::ostringstream errorMessage;
	...
	{
		const auto errorCode = GetLastError();
		GE3D_ASSERTF( false, "Couldn't start a process: %s", GetFormattedSystemMessage( errorCode ).c_str() );
		if ( o_errorMessage )
		{
			std::ostringstream errorMessage;
...
GE3D::cResult GE3D::Windows::GetFilesInDirectory( const std::string& i_path, std::vector<std::string>& o_paths,
...
GE3D::cResult GE3D::Windows::GetEnvironmentVariable( const char* const i_key, std::string& o_value, std::string* const o_errorMessage )
{
...
std::string GE3D::Windows::GetFormattedSystemMessage( const DWORD i_code )
{
...
std::string GE3D::Windows::GetLastSystemError( DWORD* const o_optionalErrorCode )
{
...
GE3D::cResult GE3D::Windows::GetLastWriteTime( const char* const i_path, uint64_t& o_lastWriteTime, std::string* const o_errorMessage )
{
...
GE3D::cResult GE3D::Windows::InvalidateLastWriteTime( const char* const i_path, std::string* const o_errorMessage )
{
	HANDLE fileHandle = INVALID_HANDLE_VALUE;
...
GE3D::cResult GE3D::Windows::LoadBinaryFile( const char* const i_path, sDataFromFile& o_data, std::string* const o_errorMessage )
{
	auto result = Results::Success;
	...
	{
		GE3D_ASSERT( o_data.data == nullptr );
		o_data.data = nullptr;
		o_data.size = 0;
	}
	...
	// Get the file's size
	{
		...
		{
			GE3D_ASSERT( fileSize_integer.QuadPart <= SIZE_MAX );
	...
	if ( o_data.data )
	{
		...
		GE3D_ASSERTF( o_data.size <= std::numeric_limits<DWORD>::max(), "This file is bigger than this code can read" );
...
void GE3D::Windows::OutputErrorMessageForVisualStudio( const char* const i_errorMessage, const char* const i_optionalFilePath,
	const unsigned int* const i_optionalLineNumber, const unsigned int* const i_optionalColumnNumber )
{
	OutputMessageForVisualStudio( "error", i_errorMessage, i_optionalFilePath, i_optionalLineNumber, i_optionalColumnNumber );
}

void GE3D::Windows::OutputWarningMessageForVisualStudio( const char* const i_errorMessage, const char* const i_optionalFilePath,
	const unsigned int* const i_optionalLineNumber, const unsigned int* const i_optionalColumnNumber )
{
	OutputMessageForVisualStudio( "warning", i_errorMessage, i_optionalFilePath, i_optionalLineNumber, i_optionalColumnNumber );
}

GE3D::cResult GE3D::Windows::WriteBinaryFile( const char* const i_path, const void* const i_data, const size_t i_size, std::string* const o_errorMessage )
{
	HANDLE fileHandle = INVALID_HANDLE_VALUE;
	...
	// Write the binary data
	{
		...
		GE3D_ASSERTF( i_size <= std::numeric_limits<DWORD>::max(), "This file is bigger than this code can write" );
