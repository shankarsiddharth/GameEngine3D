...
namespace
{
	constexpr wchar_t* const s_hiddenWindowClass_name = L"GE3D Hidden OpenGL Context Window Class";
}

// Interface
//==========

GE3D::cResult GE3D::Windows::OpenGl::CreateHiddenContextWindow( HINSTANCE& io_applicationInstance, sHiddenWindowInfo& o_info, std::string* const o_errorMessage )
{
	...
	// Create the hidden window
	{
		...
		if ( io_applicationInstance == NULL )
		{
			...
			if ( GetModuleHandleExW( flags, hackyWayOfGettingAnAddressInsideThisModule, &io_applicationInstance ) == FALSE )
			{
				...
				GE3D_ASSERTF( false, "Couldn't get the current instance handle: %s", windowsErrorMessage.c_str() );
				if ( o_errorMessage )
				{
		...
		// Register the hidden window's class
		{
			...
			if ( !o_info.windowClass )
			{
				result = Results::Failure;
				const auto windowsErrorMessage = GetLastSystemError();
				GE3D_ASSERTF( false, "Couldn't register the hidden OpenGL context window's class: %s", windowsErrorMessage.c_str() );
				if ( o_errorMessage )
				{
					std::ostringstream errorMessage;
		...
		// Create the hidden window
		{
			constexpr wchar_t* const windowName = L"GE3D Hidden OpenGL Context Window";
			...
			{
				result = Results::Failure;
				GE3D_ASSERTF( false, "Couldn't create the hidden OpenGL context window" );
				if ( o_errorMessage )
				{
	...
	// Get the device context
	{
		...
		{
			result = Results::Failure;
			GE3D_ASSERTF( false, "Couldn't get the hidden OpenGL device context" );
			if ( o_errorMessage )
			{
	...
	// Set the pixel format for the window
	{
		...
		int pixelFormatId;
		{
			...
			if ( pixelFormatId == 0 )
			{
				result = Results::Failure;
				const auto windowsErrorMessage = GetLastSystemError();
				GE3D_ASSERTF( false, "Couldn't choose the closest pixel format for the hidden OpenGL context window: %s", windowsErrorMessage.c_str() );
				if ( o_errorMessage )
				{
					std::ostringstream errorMessage;
		...
		if ( SetPixelFormat( o_info.deviceContext, pixelFormatId, &desiredPixelFormat ) == FALSE )
		{
			result = Results::Failure;
			const auto windowsErrorMessage = GetLastSystemError();
			GE3D_ASSERTF( false, "Couldn't set the desired pixel format (%i) for the hidden OpenGL context window: %s",
	...
	if ( !o_info.openGlRenderingContext )
	{
		result = Results::Failure;
		const auto windowsErrorMessage = GetLastSystemError();
		GE3D_ASSERTF( false, "Couldn't create the hidden OpenGL rendering context: %s", windowsErrorMessage.c_str() );
		if ( o_errorMessage )
		{
			std::ostringstream errorMessage;
	...
	if ( wglMakeCurrent( o_info.deviceContext, o_info.openGlRenderingContext ) == FALSE )
	{
		result = Results::Failure;
		const auto windowsErrorMessage = GetLastSystemError();
		GE3D_ASSERTF( false, "Couldn't set the current OpenGL rendering context for the hidden window: %s", windowsErrorMessage.c_str() );
		if ( o_errorMessage )
		{
			std::ostringstream errorMessage;
...
GE3D::cResult GE3D::Windows::OpenGl::FreeHiddenContextWindow( const HINSTANCE& i_applicationInstance, sHiddenWindowInfo& io_info, std::string* const o_errorMessage )
{
	auto result = Results::Success;

	if ( io_info.openGlRenderingContext )
	{
		GE3D_ASSERTF( io_info.deviceContext, "If a rendering context exists then a device context should also" );
		if ( wglGetCurrentContext() == io_info.openGlRenderingContext )
		{
			if ( wglMakeCurrent( io_info.deviceContext, NULL ) == FALSE )
			{
				...
				GE3D_ASSERTF( false, "Couldn't unset the current hidden OpenGL device context: %s", windowsErrorMessage.c_str() );
				if ( o_errorMessage )
				{
					std::ostringstream errorMessage;
		...
		if ( wglDeleteContext( io_info.openGlRenderingContext ) == FALSE )
		{
			...
			GE3D_ASSERTF( false, "Couldn't delete the hidden OpenGL rendering context: %s", windowsErrorMessage.c_str() );
			if ( o_errorMessage )
			{
				std::ostringstream errorMessage;
	...
	if ( io_info.deviceContext )
	{
		...
		{
			GE3D_ASSERTF( false, "A window handle is required to release a device context" );
		}
	}
	if ( io_info.window )
	{
		GE3D_ASSERTF( io_info.windowClass, "If a window was created then a window class should also be provided" );
		if ( DestroyWindow( io_info.window ) == FALSE )
		{
			...
			GE3D_ASSERTF( false, "Couldn't destroy the hidden OpenGL context window: %s", windowsErrorMessage.c_str() );
			if ( o_errorMessage )
			{
				std::ostringstream errorMessage;
	...
	{
		GE3D_ASSERTF( i_applicationInstance, "The provided instance handle is NULL" );
		if ( UnregisterClass( s_hiddenWindowClass_name, i_applicationInstance ) == FALSE )
		{
			...
			GE3D_ASSERTF( false, "Couldn't unregister the hidden OpenGL context window's class: %s", windowsErrorMessage.c_str() );
			if ( o_errorMessage )
			{
				std::ostringstream errorMessage;
