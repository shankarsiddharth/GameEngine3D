...
void GE3D::UserOutput::Print( const char* const i_message, ... )
{
	std::string message;
	{
		...
		if ( result_format >= 0 )
		{
			...
			{
				GE3D_ASSERTF( false, "The internal UserOutput buffer of size %u was not big enough to hold the formatted message of length %i",
		...
		{
			GE3D_ASSERTF( false, "An encoding error occurred in UserOutput for the message \"%s\"", i_message );
			std::ostringstream errorMessage;
...
GE3D::cResult GE3D::UserOutput::Initialize( const sInitializationParameters& i_initializationParameters )
{
...
GE3D::cResult GE3D::UserOutput::CleanUp()
{
	s_mainWindow = NULL;
