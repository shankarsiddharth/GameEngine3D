...
GE3D::cResult GE3D::Platform::CopyFile( const char* const i_path_source, const char* const i_path_target,
...
GE3D::cResult GE3D::Platform::CreateDirectoryIfItDoesntExist( const std::string& i_filePath, std::string* const o_errorMessage )
{
	return Windows::CreateDirectoryIfItDoesntExist( i_filePath, o_errorMessage );
}

bool GE3D::Platform::DoesFileExist( const char* const i_path, std::string* const o_errorMessage )
{
	return Windows::DoesFileExist( i_path, o_errorMessage );
}

GE3D::cResult GE3D::Platform::ExecuteCommand( const char* const i_command, int* const o_exitCode, std::string* const o_errorMessage )
{
...
GE3D::cResult GE3D::Platform::GetEnvironmentVariable( const char* const i_key, std::string& o_value, std::string* const o_errorMessage )
{
	return Windows::GetEnvironmentVariable( i_key, o_value, o_errorMessage );
}

GE3D::cResult GE3D::Platform::GetFilesInDirectory( const std::string& i_path, std::vector<std::string>& o_paths,
...
GE3D::cResult GE3D::Platform::GetLastWriteTime( const char* const i_path, uint64_t& o_lastWriteTime, std::string* const o_errorMessage )
{
	return Windows::GetLastWriteTime( i_path, o_lastWriteTime, o_errorMessage );
}

GE3D::cResult GE3D::Platform::InvalidateLastWriteTime( const char* const i_path, std::string* const o_errorMessage )
{
	return Windows::InvalidateLastWriteTime( i_path, o_errorMessage );
}

GE3D::cResult GE3D::Platform::LoadBinaryFile( const char* const i_path, sDataFromFile& o_data, std::string* const o_errorMessage )
{
...
GE3D::cResult GE3D::Platform::WriteBinaryFile( const char* const i_path, const void* const i_data, const size_t i_size, std::string* const o_errorMessage )
{
