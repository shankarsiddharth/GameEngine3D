...
namespace
{
	...
	public:
		...
		// Access
		void AddShader( const GE3D::Graphics::cShader* const i_shader );
		void RemoveShader( const GE3D::Graphics::cShader* const i_shader );
	...
	private:
		...
		std::vector<const GE3D::Graphics::cShader*> m_shaders;
		GE3D::Concurrency::cMutex m_mutex;
...
GE3D::cResult GE3D::Graphics::cShader::Load( const std::string& i_path, cShader*& o_shader, const eShaderType i_type )
{
	...
	// Validate the shader type
	{
		...
		default:
			...
			GE3D_ASSERTF( false, "Invalid shader type: %i", i_type );
	...
	// Allocate a new shader
	{
		...
		if ( !newShader )
		{
			...
			GE3D_ASSERTF( false, "Couldn't allocate memory for the shader %s", i_path.c_str() );
	...
	{
		GE3D_ASSERTF( false, "Initialization of new shader failed" );
		return result;
	}
...
GE3D::Graphics::cShader::cShader( const eShaderType i_type )
	:
	m_type( i_type )
{
...
namespace
{
	...
	void cShaderTracker::AddShader( const GE3D::Graphics::cShader* const i_shader )
	{
		GE3D_ASSERT( i_shader != nullptr );
		GE3D::Concurrency::cMutex::cScopeLock scopeLock( m_mutex );
#ifdef GE3D_ASSERTS_AREENABLED
		for ( const auto *const shader : m_shaders )
		{
			GE3D_ASSERTF( shader != i_shader, "This shader is already being tracked" );
		}
#endif
		m_shaders.push_back( i_shader );
	}

	void cShaderTracker::RemoveShader( const GE3D::Graphics::cShader* const i_shader )
	{
		GE3D::Concurrency::cMutex::cScopeLock scopeLock( m_mutex );
		...
		GE3D_ASSERTF( false, "This shader wasn't being tracked" );
	}
	...
	cShaderTracker::~cShaderTracker()
	{
		...
		{
			GE3D_ASSERTF( !haveAnyShadersNotBeenDeleted,
			...
			GE3D::Logging::OutputError( "Not every shader was released. The program will now crash!" );
