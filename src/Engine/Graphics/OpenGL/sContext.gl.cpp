...
namespace
{
	GE3D::cResult CreateRenderingContext();
}
...
GE3D::cResult GE3D::Graphics::sContext::Initialize( const sInitializationParameters& i_initializationParameters )
{
...
GE3D::cResult GE3D::Graphics::sContext::CleanUp()
{
	auto result = Results::Success;
	...
	if ( openGlRenderingContext != NULL )
	{
		if ( wglMakeCurrent( deviceContext, NULL ) != FALSE )
		{
			if ( wglDeleteContext( openGlRenderingContext ) == FALSE )
			{
				...
				GE3D_ASSERTF( false, windowsErrorMessage.c_str() );
...
namespace
{
	GE3D::cResult CreateRenderingContext()
	{
		...
		// Get the device context
		{
			...
			{
				GE3D_ASSERTF( false, "Couldn't get device context" );
				GE3D::Logging::OutputError( "Windows failed to get the device context" );
		...
		// (This can only be done _once_ for a given window)
		{
			...
			int pixelFormatId;
			{
					...
					&pixelFormatId, &returnedFormatCount ) != FALSE )
				{
					...
					{
						GE3D_ASSERTF( false, "Couldn't find a pixel format" );
						GE3D::Logging::OutputError( "Windows couldn't find a pixel format that satisfied the desired attributes" );
						return GE3D::Results::Failure;
					}
				}
				else
				{
					...
					GE3D_ASSERTF( false, windowsErrorMessage.c_str() );
					GE3D::Logging::OutputError( "Windows failed to choose the closest pixel format: %s", windowsErrorMessage.c_str() );
					return GE3D::Results::Failure;
				}
			}
			// Set it
			{
				...
				if ( SetPixelFormat( g_context.deviceContext, pixelFormatId, &pixelFormatDescriptor ) == FALSE )
				{
					const auto windowsErrorMessage = GE3D::Windows::GetLastSystemError();
					GE3D_ASSERTF( false, windowsErrorMessage.c_str() );
					GE3D::Logging::OutputError( "Windows couldn't set the desired pixel format: %s", windowsErrorMessage.c_str() );
					return GE3D::Results::Failure;
				}
			}
		}
		// Create an OpenGL rendering context and make it current
		{
			// Create the context
			{
...
#ifdef GE3D_GRAPHICS_ISDEVICEDEBUGINFOENABLED
				...
				{
					DWORD errorCode;
					const auto windowsErrorMessage = GE3D::Windows::GetLastSystemError( &errorCode );
					...
					GE3D_ASSERTF( false, errorMessage.str().c_str() );
					GE3D::Logging::OutputError( errorMessage.str().c_str() );
			...
			if ( wglMakeCurrent( g_context.deviceContext, g_context.openGlRenderingContext ) == FALSE )
			{
				const auto windowsErrorMessage = GE3D::Windows::GetLastSystemError();
				GE3D_ASSERTF( false, windowsErrorMessage.c_str() );
				GE3D::Logging::OutputError( "Windows failed to set the current OpenGL rendering context: %s",
