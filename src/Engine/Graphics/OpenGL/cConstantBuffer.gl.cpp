...
void GE3D::Graphics::cConstantBuffer::Bind( const uint_fast8_t ) const
{
	GE3D_ASSERT( m_bufferId != 0 );
...
GE3D::cResult GE3D::Graphics::cConstantBuffer::Initialize_platformSpecific( const void* const i_initialData )
{
	...
	// Create a uniform buffer object and make it active
	{
		...
		if ( errorCode == GL_NO_ERROR )
		{
			...
			if ( errorCode != GL_NO_ERROR )
			{
				result = Results::Failure;
				GE3D_ASSERTF( false, reinterpret_cast<const char*>( gluErrorString( errorCode ) ) );
				GE3D::Logging::OutputError( "OpenGL failed to bind the new uniform buffer %u: %s",
		...
		else
		{
			result = Results::Failure;
			GE3D_ASSERTF( false, reinterpret_cast<const char*>( gluErrorString( errorCode ) ) );
			GE3D::Logging::OutputError( "OpenGL failed to get an unused uniform buffer ID: %s",
				reinterpret_cast<const char*>( gluErrorString( errorCode ) ) );
			return result;
		}
	}
	// Allocate space and copy the constant data into the uniform buffer
	{
		...
		if ( errorCode != GL_NO_ERROR )
		{
			result = Results::Failure;
			GE3D_ASSERTF( false, reinterpret_cast<const char*>( gluErrorString( errorCode ) ) );
			GE3D::Logging::OutputError( "OpenGL failed to allocate the new uniform buffer %u: %s",
				m_bufferId, reinterpret_cast<const char*>( gluErrorString( errorCode ) ) );
			return result;
		}
	}
