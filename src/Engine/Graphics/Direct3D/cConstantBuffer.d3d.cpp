...
void GE3D::Graphics::cConstantBuffer::Bind( const uint_fast8_t i_shaderTypesToBindTo ) const
{
	auto* const direct3dImmediateContext = sContext::g_context.direct3dImmediateContext;
	GE3D_ASSERT( direct3dImmediateContext );

	GE3D_ASSERT( m_buffer );
...
void GE3D::Graphics::cConstantBuffer::Update( const void* const i_data )
{
	auto* const direct3dImmediateContext = sContext::g_context.direct3dImmediateContext;
	GE3D_ASSERT( direct3dImmediateContext );
	...
	void* memoryToWriteTo;
	{
		D3D11_MAPPED_SUBRESOURCE mappedSubResource;
		{
			...
			{
				GE3D_ASSERTF( false, "Couldn't map constant buffer" );
...
GE3D::cResult GE3D::Graphics::cConstantBuffer::CleanUp()
{
...
GE3D::cResult GE3D::Graphics::cConstantBuffer::Initialize_platformSpecific( const void* const i_initialData )
{
	...
	const auto bufferDescription = [this]
	{
		...
		GE3D_ASSERTF( m_size <= std::numeric_limits<unsigned int>::max(),
	...
	{
		GE3D_ASSERTF( false, "Couldn't create constant buffer creation (HRESULT %#010x)", d3dResult );
		GE3D::Logging::OutputError( "Direct3D failed to create a constant buffer with HRESULT %#010x", d3dResult );
