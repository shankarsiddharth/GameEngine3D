...
void GE3D::Graphics::cRenderState::Bind() const
{
	...
	// Depth Buffering
	{
		GE3D_ASSERT( m_depthStencilState );
...
GE3D::cResult GE3D::Graphics::cRenderState::Initialize( const uint8_t i_renderStateBits )
{
	...
	GE3D_ASSERT( direct3dDevice != nullptr );
	...
	// Alpha Transparency
	{
		...
		if ( FAILED( d3dResult ) )
		{
			...
			GE3D_ASSERTF( false, "CreateBlendState() failed (HRESULT %#010x)", d3dResult );
			GE3D::Logging::OutputError( "Direct3D failed to create a blend render state object from %u with HRESULT %#010x",
	...
	// Depth Buffering
	{
		...
		{
			result = Results::Failure;
			GE3D_ASSERTF( false, "CreateDepthStencilState() failed (HRESULT %#010x)", d3dResult );
			GE3D::Logging::OutputError( "Direct3D failed to create a depth/stencil render state object from %u with HRESULT %#010x",
				i_renderStateBits, d3dResult );
			return result;
		}
	}
	// Draw Both Triangle Sides
	{
		...
		{
			result = Results::Failure;
			GE3D_ASSERTF( false, "CreateRasterizerState() failed (HRESULT %#010x)", d3dResult );
			GE3D::Logging::OutputError( "Direct3D failed to create a rasterizer render state object from %u with HRESULT %#010x",
				i_renderStateBits, d3dResult );
			return result;
		}
	}
