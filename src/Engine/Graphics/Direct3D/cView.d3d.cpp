...
GE3D::cResult GE3D::Graphics::cView::Initialize(const unsigned int i_resolutionWidth/* = 0*/, const unsigned int i_resolutionHeight/* = 0*/)
{
	auto result = GE3D::Results::Success;

	ID3D11Texture2D* backBuffer = nullptr;
	ID3D11Texture2D* depthBuffer = nullptr;
	GE3D::cScopeGuard scopeGuard([&backBuffer, &depthBuffer]
		{
	...
	auto& g_context = GE3D::Graphics::sContext::g_context;
	...
	// meaning a texture that the GPU can render to)
	{
		// Get the back buffer from the swap chain
		{
			...
			if (FAILED(d3dResult))
			{
				...
				GE3D_ASSERTF(false, "Couldn't get the back buffer from the swap chain (HRESULT %#010x)", d3dResult);
				GE3D::Logging::OutputError("Direct3D failed to get the back buffer from the swap chain (HRESULT %#010x)", d3dResult);
				return result;
			}
		}
		// Create the view
		{
			...
			{
				result = GE3D::Results::Failure;
				GE3D_ASSERTF(false, "Couldn't create render target view (HRESULT %#010x)", d3dResult);
				GE3D::Logging::OutputError("Direct3D failed to create the render target view (HRESULT %#010x)", d3dResult);
				return result;
			}
		}
	}
	// Create a depth/stencil buffer and a view of it
	{
		// Unlike the back buffer no depth/stencil buffer exists until and unless it is explicitly created
		{
			const auto textureDescription = [i_resolutionWidth, i_resolutionHeight]
			{
				auto textureDescription = []() constexpr
				{
					...
					textureDescription.Format = DXGI_FORMAT_D24_UNORM_S8_UINT;	// 24 bits for depth and 8 bits for stencil
					{
						...
						sampleDescription.Count = 1;	// No multi-sampling
			...
			{
				result = GE3D::Results::Failure;
				GE3D_ASSERTF(false, "Couldn't create depth buffer (HRESULT %#010x)", d3dResult);
				GE3D::Logging::OutputError("Direct3D failed to create the depth buffer resource (HRESULT %#010x)", d3dResult);
				return result;
			}
		}
		// Create the view
		{
			...
			{
				result = GE3D::Results::Failure;
				GE3D_ASSERTF(false, "Couldn't create depth stencil view (HRESULT %#010x)", d3dResult);
				GE3D::Logging::OutputError("Direct3D failed to create the depth stencil view (HRESULT %#010x)", d3dResult);
				return result;
			}
		}
	}
...
void GE3D::Graphics::cView::Clear(float i_red /*= 0.0f*/, float i_green /*= 0.0f*/, float i_blue /*= 0.0f*/, float i_alpha /*= 1.0f*/)
{
	...
	{
		GE3D_ASSERT(m_renderTargetView);

		// Black is usually used
		float clearColor[4] = { i_red, i_green, i_blue, i_alpha };
	...
	{
		GE3D_ASSERT(m_depthStencilView);
