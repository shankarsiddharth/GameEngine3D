...
namespace
{
	GE3D::cResult CreateDevice( const unsigned int i_resolutionWidth, const unsigned int i_resolutionHeight );
}
...
GE3D::cResult GE3D::Graphics::sContext::Initialize( const sInitializationParameters& i_initializationParameters )
{
	...
	{
		GE3D_ASSERTF( false, "Can't initialize context without device" );
		return result;
	}
...
namespace
{
	GE3D::cResult CreateDevice( const unsigned int i_resolutionWidth, const unsigned int i_resolutionHeight )
	{
		...
		{
			GE3D_ASSERT( false );
			GE3D::Logging::OutputError( "Direct3D failed to create a Direct3D11 device with HRESULT %#010x", result_create );
