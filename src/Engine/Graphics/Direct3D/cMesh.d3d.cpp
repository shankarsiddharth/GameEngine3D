...
GE3D::cResult GE3D::Graphics::cMesh::Initialize(GE3D::Graphics::VertexFormats::sVertex_mesh* i_vertexArray, size_t i_vertexArraySize, uint16_t* i_indexArray, size_t i_indexArraySize)
{
	...
	auto* const direct3dDevice = GE3D::Graphics::sContext::g_context.direct3dDevice;
	GE3D_ASSERT(direct3dDevice);

	// Vertex Format
	{
		if (!(result = GE3D::Graphics::cVertexFormat::Load(GE3D::Graphics::eVertexType::Mesh, m_vertexFormat,
			"data/Shaders/Vertex/vertexInputLayout_mesh.bsasset")))
		{
			GE3D_ASSERTF(false, "Can't initialize geometry without vertex format");
			return result;
		}
	}
	// Vertex Buffer
	{
		...
		GE3D_ASSERT(bufferSize <= std::numeric_limits<decltype(D3D11_BUFFER_DESC::ByteWidth)>::max());
		...
		if (FAILED(result_create))
		{
			...
			GE3D_ASSERTF(false, "3D object vertex buffer creation failed (HRESULT %#010x)", result_create);
			GE3D::Logging::OutputError("Direct3D failed to create a 3D object vertex buffer (HRESULT %#010x)", result_create);
			return result;
		}		
	}
	...
	// Index Buffer
	{
		...
		GE3D_ASSERT(m_indexBufferSize != 0)
		GE3D_ASSERT(m_indexBufferSize % indexCountPerTriangle == 0)		
		...
		{
			result = GE3D::Results::Failure;
			GE3D_ASSERTF(false, "3D object index buffer creation failed (HRESULT %#010x)", result_create);
			GE3D::Logging::OutputError("Direct3D failed to create a 3D object index buffer (HRESULT %#010x)", result_create);
			return result;
		}
	}
...
void GE3D::Graphics::cMesh::Draw()
{
	...
	// Draw the geometry
	{
		...
		{
			GE3D_ASSERT(m_vertexBuffer != nullptr);
		...
		// Specify what kind of data the vertex buffer holds
		{
			...
			{
				GE3D_ASSERT(m_vertexFormat != nullptr);
				m_vertexFormat->Bind();
			}
		...
		{
			GE3D_ASSERT(m_indexBuffer != nullptr);
