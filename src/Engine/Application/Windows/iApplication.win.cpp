...
namespace
{
	...
	GE3D::cResult CreateMainWindow( const HINSTANCE i_thisInstanceOfTheApplication, const char* const i_windowName, const ATOM i_windowClass,
		const uint16_t i_resolutionWidth, const uint16_t i_resolutionHeight,
		GE3D::Application::iApplication& io_application, HWND& o_window );
	GE3D::cResult CreateMainWindowClass( const HINSTANCE i_thisInstanceOfTheApplication, const char* const i_mainWindowClassName,
	...
	GE3D::cResult FreeMainWindow( HWND& io_window );
	GE3D::cResult FreeMainWindowClass( const HINSTANCE i_thisInstanceOfTheApplication, ATOM& io_mainWindowClass );
}
...
GE3D::cResult GE3D::Application::iApplication::GetCurrentResolution( uint16_t& o_width, uint16_t& o_height ) const
{
...
GE3D::Application::iApplication::iApplication()
	:
...
GE3D::Application::iApplication* GE3D::Application::iApplication::GetApplicationFromWindow( const HWND i_window )
{
	const auto userData = GetWindowLongPtr( i_window, GWLP_USERDATA );
	auto* const application = reinterpret_cast<iApplication*>( userData );
	GE3D_ASSERT( application );
	GE3D_ASSERT( application->m_mainWindow == i_window );
	return application;
}
...
GE3D::cResult GE3D::Application::iApplication::RenderFramesWhileWaitingForApplicationToExit( int& o_exitCode )
{
...
GE3D::cResult GE3D::Application::iApplication::Exit_platformSpecific( const int i_exitCode )
{
...
LRESULT CALLBACK GE3D::Application::iApplication::OnMessageReceivedFromWindows( HWND i_window, UINT i_message, WPARAM i_wParam, LPARAM i_lParam )
{
	...
	case WM_NCCREATE:
		{
			...
			auto* const application =
				static_cast<GE3D::Application::iApplication*>( creationData.lpCreateParams );
			GE3D_ASSERT( application != nullptr );
			...
			// Assign the application to the new window's user data
			{
				...
				if ( previousUserData == NULL )
				{
					...
					{
						GE3D_ASSERTF( "Couldn't set main window user data: %s", errorMessage.c_str() );
...
GE3D::cResult GE3D::Application::iApplication::Initialize_base( const sEntryPointParameters& i_entryPointParameters )
{
	...
	{
		GE3D_ASSERTF( false, "Main window class wasn't created" );
		return result;
	}
	// Create the main window
	{
			...
			*this, window ) )
		{
			...
			GE3D_ASSERT( window == m_mainWindow );
		...
		{
			GE3D_ASSERTF( false, "Main window wasn't created" );
			return result;
		}
	}
...
GE3D::cResult GE3D::Application::iApplication::PopulateGraphicsInitializationParameters( Graphics::sInitializationParameters& o_initializationParameters )
{
	GE3D_ASSERT( m_mainWindow != NULL );
...
GE3D::cResult GE3D::Application::iApplication::PopulateUserOutputInitializationParameters( UserOutput::sInitializationParameters& o_initializationParameters )
{
	GE3D_ASSERT( m_mainWindow != NULL );
	o_initializationParameters.mainWindow = m_mainWindow;
	return Results::Success;
...
GE3D::cResult GE3D::Application::iApplication::CleanUp_base()
{
	auto result = Results::Success;
...
namespace
{
		...
		GE3D::Application::iApplication& io_application, HWND& o_window )
	{
		...
		GE3D::cScopeGuard scopeGuard( [&o_window, &result]
			{
				if ( !result )
				{
					if ( o_window != NULL )
		...
		// Create the window
		{
			...
			o_window = CreateWindowExW( windowStyle_extended, MAKEINTATOM( i_windowClass ),
				GE3D::Windows::ConvertUtf8ToUtf16( i_windowName ).c_str(), windowStyle,
				position_x, position_y, width, height,
				parent, menu, i_thisInstanceOfTheApplication, userData );
			if ( o_window != NULL )
			{
				GE3D::Logging::OutputMessage( "Created main window \"%s\"", i_windowName );
			}
			else
			{
				result = GE3D::Results::Failure;
				const auto errorMessage = GE3D::Windows::GetLastSystemError();
				GE3D_ASSERTF( false, "Main window wasn't created: %s", errorMessage.c_str() );
				GE3D::Logging::OutputError( "Windows failed to create the main window: %s", errorMessage.c_str() );
				return result;
			}
		}
		...
		// Change the window's size based on the desired client area resolution
		{
			...
			} nonClientAreaSize;
			{
				...
				if ( GetWindowRect( o_window, &windowCoordinates ) == FALSE )
				{
					result = GE3D::Results::Failure;
					const auto errorMessage = GE3D::Windows::GetLastSystemError();
					GE3D_ASSERTF( false, "Couldn't get coordinates of the main window: %s", errorMessage.c_str() );
					GE3D::Logging::OutputError( "Windows failed to get the coordinates of the main window: %s", errorMessage.c_str() );
					return result;
				}
				...
				if ( GetClientRect( o_window, &clientDimensions ) == FALSE )
				{
					result = GE3D::Results::Failure;
					const auto errorMessage = GE3D::Windows::GetLastSystemError();
					GE3D_ASSERTF( false, "Couldn't get the main window's client dimensions: %s", errorMessage.c_str() );
					GE3D::Logging::OutputError( "Windows failed to get the dimensions of the main window's client area:: %s",
						errorMessage.c_str() );
					return result;
				}
			...
			// Resize the window
			{
				...
				{
					GE3D::Logging::OutputMessage( "Set main window resolution to %u x %u",
				...
				else
				{
					result = GE3D::Results::Failure;
					const auto errorMessage = GE3D::Windows::GetLastSystemError();
					GE3D_ASSERTF( false, "Couldn't resize the main window to &i x &i: %s",
						desiredWidth_window, desiredHeight_window, errorMessage.c_str() );
					GE3D::Logging::OutputError( "Windows failed to resize main window to &i x &i"
		...
		const WORD* const i_iconId_large, const WORD* const i_iconId_small )
	{
		...
		const auto mainWindowClassName = GE3D::Windows::ConvertUtf8ToUtf16( i_mainWindowClassName );
		{
		...
		{
			GE3D::Logging::OutputMessage( "Registered main window class \"%s\"", i_mainWindowClassName );
			return GE3D::Results::Success;
		}
		else
		{
			const auto errorMessage = GE3D::Windows::GetLastSystemError();
			GE3D_ASSERTF( false, "Main window class registration failed: %s", errorMessage.c_str() );
			GE3D::Logging::OutputError( "Windows failed to register the main window class: %s", errorMessage.c_str() );
	...
	GE3D::cResult FreeMainWindow( HWND& io_window )
	{
		...
		{
			const auto errorMessage = GE3D::Windows::GetLastSystemError();
			GE3D_ASSERTF( false, "Main window wasn't destroyed: %s", errorMessage.c_str() );
			GE3D::Logging::OutputError( "Windows failed to destroy the main window: %s", errorMessage.c_str() );
			return GE3D::Results::Failure;
		}
	}
	...
	GE3D::cResult FreeMainWindowClass( const HINSTANCE i_thisInstanceOfTheApplication, ATOM& io_mainWindowClass  )
	{
		...
		{
			GE3D::Logging::OutputMessage( "Unregistered main window class" );
		...
		{
			const auto errorMessage = GE3D::Windows::GetLastSystemError();
			GE3D_ASSERTF( false, "Main window class wasn't unregistered: %s", errorMessage.c_str() );
			GE3D::Logging::OutputError( "Windows failed to unregister the main window class: %s", errorMessage.c_str() );
			return GE3D::Results::Failure;
		}
	}
}
