#ifndef GE3D_ASSETS_CMANAGER_INL
#define GE3D_ASSETS_CMANAGER_INL
...
tAsset* GE3D::Assets::cManager<tAsset, tKey>::Get( const cHandle<tAsset> i_handle )
{
	GE3D_ASSERTF( i_handle, "This handle is invalid (it has never been associated with a valid asset)" );
	...
	Concurrency::cMutex::cScopeLock autoLock( m_mutex );
	{
		...
		if ( index < assetCount )
		{
			...
			{
				GE3D_ASSERTF( false, "A handle (at index %u) has an ID (%u) that doesn't match the asset record (%u)",
					index, id_handle, id_assetRecord );
			}
		}
		else
		{
			GE3D_ASSERTF( false, "A handle has an index (%u) that's too big for the number of assets (%u)",
				index, assetCount );
		}
	}
...
GE3D::cResult GE3D::Assets::cManager<tAsset, tKey>::Load( const tKey& i_key, cHandle<tAsset>& o_handle, tConstructorArguments&&... i_constructorArguments )
{
	GE3D_ASSERTF( !o_handle, "The output handle isn't invalid (this probably indicates that a resource leak is about to happen)" );
			...
			if ( iterator != m_map_keysToHandles.end() )
			{
				...
				if ( index < assetCount )
				{
					...
					if ( id_handle == id_assetRecord )
					{
						...
						{
							GE3D_ASSERTF( false, "An asset has been loaded too many times (the manager's reference count is too big)" );
	...
	{
		GE3D_ASSERT( newAsset != nullptr );
		if ( result = GetHandleForNewAsset( *newAsset, o_handle ) )
		{
			GE3D_ASSERT( o_handle );
			// Lock the collections
			Concurrency::cMutex::cScopeLock autoLock( m_mutex );
			{
	...
	template <class tAsset, class tKey>
GE3D::cResult GE3D::Assets::cManager<tAsset, tKey>::Release( cHandle<tAsset>& io_handle )
{
	auto result = Results::Success;

	GE3D_ASSERTF( io_handle, "This handle is invalid (it has never been associated with a valid asset)" );
	// Lock the collections
	{
		Concurrency::cMutex::cScopeLock autoLock( m_mutex );
		{
			...
			if ( index < assetCount )
			{
				...
				if ( id_handle == id_assetRecord )
				{
					...
					GE3D_ASSERT( assetRecord.referenceCount > 0 );
				...
				{
					GE3D_ASSERTF( false, "A handle attempting to be released (at index %u) has an ID (%u) that doesn't match the asset record (%u)",
						index, id_handle, id_assetRecord );
			...
			{
				GE3D_ASSERTF( false, "A handle attempting to be released has an index (%u) that's too big for the number of assets (%u)",
					index, assetCount );
				result = Results::Failure;
			}
		}
	...
	template <class tAsset, class tKey>
GE3D::cResult GE3D::Assets::cManager<tAsset, tKey>::Duplicate( const cHandle<tAsset> i_handle, cHandle<tAsset>& o_handle )
{
	auto result = Results::Success;
	...
	if ( existingAsset )
	{
		...
		{
			GE3D_ASSERT( duplicatedAsset != nullptr );
...
//--------------
	...
	template <class tAsset, class tKey>
tAsset* GE3D::Assets::cManager<tAsset, tKey>::UnsafeGet( const uint_fast32_t i_index )
{
	// Lock the collections
	Concurrency::cMutex::cScopeLock autoLock( m_mutex );
	{
		GE3D_ASSERT( i_index < m_assetRecords.size() );
		return m_assetRecords[i_index].asset;
	}
}

	template <class tAsset, class tKey>
void GE3D::Assets::cManager<tAsset, tKey>::UnsafeIncrementReferenceCount( const uint_fast32_t i_index )
{
	// Lock the collections
	Concurrency::cMutex::cScopeLock autoLock( m_mutex );
	{
	...
	template <class tAsset, class tKey>
void GE3D::Assets::cManager<tAsset, tKey>::UnsafeDecrementReferenceCount( const uint_fast32_t i_index )
{
	// Lock the collections
	Concurrency::cMutex::cScopeLock autoLock( m_mutex );
	{
...
//----------------------
	...
	template <class tAsset, class tKey>
GE3D::cResult GE3D::Assets::cManager<tAsset, tKey>::Initialize()
{
	return Results::Success;
}

	template <class tAsset, class tKey>
GE3D::cResult GE3D::Assets::cManager<tAsset, tKey>::CleanUp()
{
	auto result = Results::Success;
		...
		// Lock the collections
		{
			Concurrency::cMutex::cScopeLock autoLock( m_mutex );
			{
				for ( auto& assetRecord : m_assetRecords )
				{
					...
					{
						GE3D_ASSERTF( false, "A manager still has a record of an asset that hasn't been released" );
						result = Results::Failure;
	...
	template <class tAsset, class tKey>
GE3D::Assets::cManager<tAsset, tKey>::~cManager<tAsset, tKey>()
{
...
//===============
	...
	template <class tAsset, class tKey>
GE3D::Assets::cManager<tAsset, tKey>::sAssetRecord::sAssetRecord( tAsset* const i_asset, const uint16_t i_id, const uint16_t i_referenceCount )
	:
...
//----------------------
	...
	template <class tAsset, class tKey>
GE3D::cResult GE3D::Assets::cManager<tAsset, tKey>::GetHandleForNewAsset( tAsset& i_asset, cHandle<tAsset>& o_handle )
{
	GE3D_ASSERTF( !o_handle, "The output handle isn't invalid (this probably indicates that a resource leak is about to happen)" );
	...
	else
	{
		...
		{
			GE3D_ASSERTF( false, "Too many of this kind of asset have been created" );
	...
	template <class tAsset, class tKey>
void GE3D::Assets::cManager<tAsset, tKey>::OnAssetReferenceCountDecrementedToZero( const uint_fast32_t i_index )
{
	// The mutex should be locked when this function is called
	GE3D_ASSERT( m_assetRecords[i_index].referenceCount == 0 );
...
#endif	// GE3D_ASSETS_CMANAGER_INL
