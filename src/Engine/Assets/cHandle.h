...
#ifndef GE3D_ASSETS_CHANDLE_H
#define GE3D_ASSETS_CHANDLE_H
...
#define GE3D_ASSETS_INDEX_MASK 0Xfffff	// The lowest 20 bits
			uint_fast32_t GetIndex() const { return static_cast<uint_fast32_t>( m_value & GE3D_ASSETS_INDEX_MASK ); }
		...
		private:
			...
			static constexpr uint_fast32_t InvalidIndex = GE3D_ASSETS_INDEX_MASK;
...
#define GE3D_ASSETS_ID_SHIFT 20	// The remaining high bits
#define GE3D_ASSETS_ID_MAX ( 1 << ( ( sizeof( cHandle<void*> ) * 8 ) - GE3D_ASSETS_ID_SHIFT ) ) - 1

			uint_fast16_t GetId() const { return static_cast<uint_fast16_t>( m_value >> GE3D_ASSETS_ID_SHIFT ); }
			...
			static constexpr uint_fast16_t IncrementId( const uint_fast16_t i_id )
			{
				...
				return ( i_id + 1 ) & GE3D_ASSETS_ID_MAX;
			}
			...
			cHandle( const uint_fast32_t i_index, const uint_fast16_t i_id )
				:
				m_value( i_index | ( i_id << GE3D_ASSETS_ID_SHIFT ) )
			{
				static_assert( ( ( sizeof( cHandle<void*> ) * 8 ) - GE3D_ASSETS_ID_SHIFT ) <= 16,
					"A handle's error checking ID must fit into 16 bits" );
			}

#undef GE3D_ASSETS_INDEX_MASK
#undef GE3D_ASSETS_ID_SHIFT
#undef GE3D_ASSETS_ID_MAX
