...
#ifndef GE3D_ASSETS_REFERENCECOUNTEDASSETS_H
#define GE3D_ASSETS_REFERENCECOUNTEDASSETS_H
...
//-------------------
	...
	#define GE3D_ASSETS_DECLAREREFERENCECOUNTINGFUNCTIONS()	\
		void IncrementReferenceCount()	\
		{	\
			GE3D_ASSERT( ( m_referenceCount > 0 ) && ( m_referenceCount < std::numeric_limits<decltype( m_referenceCount )>::max() ) );	\
			GE3D_ASSERT( ( reinterpret_cast<uintptr_t>( &m_referenceCount ) % 2 ) == 0 );	\
...
#define GE3D_ASSETS_DECLAREDELETEDREFERENCECOUNTEDFUNCTIONS( tClassName )	\
...
//=====
	...
	#define GE3D_ASSETS_DECLAREREFERENCECOUNT() uint16_t m_referenceCount = 1;

#else
	#error "No implementation exists for reference counting on this platform"
#endif

#endif	// GE3D_ASSETS_REFERENCECOUNTEDASSETS_H
