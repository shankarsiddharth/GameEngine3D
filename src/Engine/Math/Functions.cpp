...
uint16_t GE3D::Math::ConvertFloatToHalf( const float i_value )
{
	...
	const auto bitRepresentation = [i_value]
	{
		...
		{
			GE3D_ASSERT( sizeof( bitRepresentation ) == sizeof( i_value ) );
...
float GE3D::Math::ConvertHorizontalFieldOfViewToVerticalFieldOfView( const float i_horizontalFieldOfView_inRadians,
