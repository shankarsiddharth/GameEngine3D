...
void eae6320::Graphics::cFrameBuffer::Bind( const uint_fast8_t i_shaderTypesToBindTo ) const
{
...
void eae6320::Graphics::cFrameBuffer::Update( const void* const i_data )
{
	...
	void* memoryToWriteTo;
	{
		D3D11_MAPPED_SUBRESOURCE mappedSubResource;
		{
			...
			{
				EAE6320_ASSERTF( false, "Couldn't map frame buffer" );
				Logging::OutputError( "Direct3D failed to map a frame buffer" );
				return;
			}
		}
...
eae6320::cResult eae6320::Graphics::cFrameBuffer::Initialize_platformSpecific( const void* const i_initialData )
{
	...
	const auto bufferDescription = [this]
	{
		...
		EAE6320_ASSERTF( m_size <= std::numeric_limits<unsigned int>::max(),
			"The frame buffer format's size (%u) is too large to fit into a D3D11_BUFFER_DESC", m_size );
	...
	{
		EAE6320_ASSERTF( false, "Couldn't create frame buffer creation (HRESULT %#010x)", d3dResult );
		eae6320::Logging::OutputError( "Direct3D failed to create a frame buffer with HRESULT %#010x", d3dResult );
